...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  __cs149_vec_float x;
  __cs149_vec_int y, zeroes, ones;
  __cs149_vec_float result;
  __cs149_vec_float zero = _cs149_vset_float(0.f);
  __cs149_mask maskAll, maskIsZero, maskToExp, isLarge;
  __cs149_vec_float large;
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    _cs149_vset_float(large, 9.999999f, maskAll);
    _cs149_vset_int(zeroes, 0, maskAll);
    // All zeros
    ...
    _cs149_veq_int(maskIsZero, y, zeroes, maskAll); // if y == 0

    _cs149_vset_float(x, 1.f, maskIsZero); // if x == 0, set to 1.f
   
    maskToExp = _cs149_mask_not(maskIsZero); // Find which lanes are not 0

    maskToExp = _cs149_mask_and(maskToExp, maskAll); // find not 0 and active. We exponent these
   
    __cs149_vec_float result = x;
    __cs149_vec_int count = y; 
    _cs149_vset_int(ones, 1, maskToExp);
    // This need to be a vec of 1's ? mb can use maskToExp lmao
    _cs149_vsub_int(count, count, ones, maskToExp); // count = y - 1
    _cs149_vgt_int(maskToExp, count, zeroes, maskToExp);

    while (_cs149_cntbits(maskToExp)) { // While there are still things to exp
      _cs149_vmult_float(result, result, x, maskToExp);

      _cs149_vsub_int(count, count, ones, maskToExp); // count = count - 1
      
      //maskToExp = _cs149_mask_and(maskToExp, count); // figure out which lanes still need to be exp
      _cs149_vgt_int(maskToExp, count, zeroes, maskToExp);
    }

    _cs149_vgt_float(isLarge, result, large, maskAll); // if result > 9,999999f

    _cs149_vset_float(result, 9.999999f, isLarge); // set result = 9.999999f
